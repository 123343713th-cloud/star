...
class AuthPageUI {
    ...
    async retryAction(action, isNetworkError, attempts = 2, delayMs = 800) {
        try {
            return await action();
        } catch (e) {
            if (isNetworkError(e) && attempts > 0) {
                await new Promise(r => setTimeout(r, delayMs));
                return await this.retryAction(action, isNetworkError, attempts - 1, delayMs * 1.5);
            }
            throw e;
        }
    }
    ...
    async handleLogin(event) {
        ...
        if (!this.supabase || !this.supabase.auth) {
            this.supabase = window.supabase;
            if (!this.supabase || !this.supabase.auth) {
                this.showMessage('系统连接尚未就绪，请稍后重试', 'error');
                return;
            }
        }
        ...
        try {
            ...
            await this.retryAction(loginAction, isNetworkError);
            ...
            if (msgLower.includes('failed to fetch')) {
                errorMessage = '网络连接异常或被拦截，请检查网络后重试';
            }
    ...
    async handleRegister(event) {
        ...
        try {
            ...
            const data = await this.retryAction(registerAction, isNetworkError);
            ...
            const msgLower = String(error?.message || '').toLowerCase();
            if (msgLower.includes('failed to fetch')) {
                errorMessage = '网络连接异常或被拦截，请检查网络或更换网络环境';
            }
